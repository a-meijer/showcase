...
def loadPlayers(ranks, directory):
    ...
    for filepath in Path(directory).glob("*.csv"):
        ranks[filepath.stem] = loadPlayer(ranks, filepath)

    return ranks

def loadPlayer(ranks, filepath):
    flag = 0
    with open(filepath, 'r') as file:
        ...
        for row in csv_reader:
            if flag == 0:
                ...
                rating = int(row[3])
                rank = int(row[4])
                highestRank = int(row[5])
                previousRank = int(row[6])
    ...
    with open(matchfile, 'r') as file:
        ...
        for row in csv_reader:
            ...
            if not playerfile.exists():
                ...
                addedPlayer = player.Player(row[1], row[2], 0, 0, 1000, save.numberPlayers, save.numberPlayers, save.numberPlayers, 1000, 1000)  # Default rating of 1000
            ...
            if not playerfile2.exists():
                # Create a new player object
                # name, id, wins, losses, rating, rank, highestRank, previousRank, highestRating, previousRating
                addedPlayer = player.Player(row[3], row[4], 0, 0, 1000, save.numberPlayers, save.numberPlayers, save.numberPlayers, 1000, 1000)  # Default rating of 1000
                # Load the new player into the ranks datastructure
